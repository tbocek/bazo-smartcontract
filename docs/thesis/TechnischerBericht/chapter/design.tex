...
\raggedright
\textbf{Rationale} &
To make contract creation available for account owners, it needs to be possible to create a contract account with any valid key pair of a Bazo account. Also the \textit{P2P} package would have to support the features implemented during this work.
Both tasks by themselves would have exceeded the scope of this thesis.\end{tabular}
...
\raggedright
\textbf{Restrictions} &
As the features are not yet implemented in the client nor the \textit{P2P} package, it is not possible to send this kind of transaction via the Web API of the miner. It is also not yet possible to check the result, since smart contracts are not part of the block explorer. \\ \\
