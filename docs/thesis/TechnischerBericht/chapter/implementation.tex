...
\begin{description}
  ...
  \item[2. Decode] In this step the instruction is matched with pre-defined opcodes, which can be interpreted by switch statement of the \mintinline{go}{Exec()} function. Within this section of the function, the costs for the instruction are subtracted.
...
\begin{description}
  ...
  \item[Contract] This field is the smart contract itself and contains the byte code. The data type is \mintinline{yaml}{[]byte}, so it can be packed into a transaction field.
...
Figure \ref{tokenstruct} shows the Token struct. The tokenType field can be \mintinline{go}{OPCODE} (Value: 0), \mintinline{go}{BYTES} (Value: 1), \mintinline{go}{BYTE} (Value: 2), \mintinline{go}{ADDR} (Value: 3), \mintinline{go}{LABEL} (Value: 4) which are all int constants. The value field contains the argument passed to the token. \\ \\
\end{tabular}
...
\end{minted}
\end{minipage}
\caption{Token set of contract shown in \ref{basiccontract_design}}
\label{tokenset}
\end{figure}
...
Figure \ref{compiledbytecode} shows the resulting instruction set.
\end{tabular}
...
\end{minted}
\end{minipage}
\caption{Contract compiled to \flqq Bazo Byte Code\frqq}
