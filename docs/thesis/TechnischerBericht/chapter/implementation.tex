...
This chapter focuses on explaining why things were implemented in a certain manner. In this chapter the trade-offs and decisions which were made while building the VM are documented. Since this should be useful for follow-up work, it was done in a way that makes it easier to decide if something needs to be changed or to help them reach the same conclusion faster.
...
As underlying data structure of "Stack", keeping in mind to keep the code of the VM as short and simple as possible and without unnecessary conversions it was decided to use a two dimensional byte array. \\ \\ 

\textbf{Data Type} &
It was important that the datatype used for the underlying data structure of stack was of arbitrary length. Splitting up an element into multiple bytes so that it can be saved when using an array of fixed length as underlying data structure would have been a lot more complex and more error-prone to implement. \\ \\
...
\begin{description}
  ...
  \item[2. Decode] The instruction is interpreted by the decoder. The decoder matches the instructions with pre-defined opcodes, which can be interpreted by the \mintinline{go}{Exec()} function. Within this section of the function, the costs for the instruction are deducted.
