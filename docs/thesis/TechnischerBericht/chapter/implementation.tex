...
This section describes the \mintinline{go}{Exec()} function shown in figure \ref{vmexecutioncycle}. The instructio cycle can be described as an end-less loop with a switch statement that interprets the instructions one after another and acts accordingly.
...
\subsection{Context}
For the VM execution to have any effect it is necessary to change the state of the miner eventually. This section describes how the state changes a contract execution makes are persisted.
